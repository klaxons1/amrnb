...
public final class LspFns {
    ...
    public static void Lsf_lsp(short[] lsf, int lsfOff, short[] lsp, int lspOff, int m, int[] pOverflow) {
        for (int i = 0; i < m; i++) {
            ...
            /* lsp[i] = Tbls.table[ind] + ((Tbls.table[ind+1]-Tbls.table[ind])*offset) / 256 */
            final int L_tmp = ((Tbls.table[ind + 1] - Tbls.table[ind]) * offset) >> 8;
            lsp[lspOff + i] = (short) ((Tbls.table[ind] + ((L_tmp << 16) >> 16)) << 16 >> 16);
        }
    }
    ...
    public static void Lsp_lsf(short[] lsp, int lspOff, short[] lsf, int lsfOff, int m, int[] pOverflow) {
        ...
        for (int i = m - 1; i >= 0; i--) {
            ...
            while (Tbls.table[ind] < temp) {
                ind--;
            }

            /* acos(lsp[i]) = ind*256 + ((lsp[i]-Tbls.table[ind]) * Tbls.slope[ind])/4096 */
            int L_tmp = (temp - Tbls.table[ind]) * Tbls.slope[ind];
